...
fn main() {
    let matches = App::new("USBMaker")
        ...
        .subcommands(vec![
            ...
            SubCommand::with_name("iso")
                .author(crate_authors!())
                .version(crate_version!())
                .about("Create a bootable USB drive using the contents of an ISO file")
                .args(&[
                    Arg::with_name("device")
                        .takes_value(true)
                        .required(true)
    ...
    match matches.subcommand() {
        ...
        ("iso", Some(sub_matches)) => {
            let device: PathBuf =
                PathBuf::from(sub_matches.value_of("device").expect("No device specified"));

            let iso: PathBuf =
                PathBuf::from(sub_matches.value_of("iso").expect("No device specified"));
